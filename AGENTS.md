...
- Primary active app is `apps/nomad-go-v2` (dev via `npm run dev:v2`): Next.js App Router in `app/` (routes, server actions), shared code in `src/` (components, hooks, lib); siblings include nomad-go-main, nomad-go-pwa, nomad-go-v3. Uses Drizzle (`db/schema.ts`); introspect from repo root via `npm run db:pull:v2` (not Prisma).
