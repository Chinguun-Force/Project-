...
import {
  getToursAction,
  getMarketplaceCompaniesAction,
  type MarketplaceCompany,
} from "@/app/actions/gameActions";
import { useRouter } from "next/navigation";
import { Calendar, Clock, MapPin, ArrowRight, Building2 } from "lucide-react";
...
export default function TourPlansPage() {
  ...
  useEffect(() => {
    (async () => {
      setIsLoading(true);
      const [plans, marketplaceCompanies] = await Promise.all([
        getToursAction(),
        getMarketplaceCompaniesAction(),
      ]);
      setTours(plans);
      setCompanies(marketplaceCompanies);
      setIsLoading(false);
    })();
  }, []);
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        {isLoading ? (
          ...
          <>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-5 flex flex-col flex-grow">
                      ...
                      <CompanyOperatorRow company={tour.company} />

                      <div className="flex items-center gap-4 text-sm text-[#A0A0B0] pt-4 border-t border-[#322F36] mt-3">
            ...
            {companies.length > 0 && (
              <section className="mt-16 pt-12 border-t border-[#322F36]">
                <div className="flex items-center gap-2 mb-2">
                  <Building2 className="w-5 h-5 text-[#F4C64D]" />
                  <h2 className="text-2xl font-bold text-white">Travel companies</h2>
                </div>
                <p className="text-[#A0A0B0] mb-8 max-w-2xl">
                  Meet the local operators behind these expeditions — each company runs its
                  own guided journeys across Mongolia.
                </p>
