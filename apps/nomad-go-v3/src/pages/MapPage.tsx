  ...
  return (
    <motion.div variants={pageVariants} initial="initial" animate="animate" exit="exit" className="pb-24">
      <div className="px-4 pt-4">
        <h1 className="font-display text-[26px] font-black text-ng-tx mb-1">Explore Map</h1>
        <p className="text-[13px] text-[var(--mu)] mb-3">Tap an aimag to discover camps & quests</p>
      </div>

      {/* Toggle */}
      <div className="mx-4 mb-3 flex bg-[var(--bg3)] rounded-full p-1">
        <button
          onClick={() => setViewMode('aimag')}
          className={`flex-1 text-[11px] font-bold py-1.5 rounded-full transition-all ${
            viewMode === 'aimag' ? 'bg-white text-ng-tx shadow-sm' : 'text-[var(--mu)]'
          }`}
        >
          Аймаг
        </button>
        <button
          ...
          className={`flex-1 text-[11px] font-bold py-1.5 rounded-full transition-all ${
            viewMode === 'camp' ? 'bg-white text-ng-tx shadow-sm' : 'text-[var(--mu)]'
          }`}
        >
          Кемп
        </button>
