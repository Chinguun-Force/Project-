...
export default function MongoliaMap({ selected, onSelect, viewMode }: MongoliaMapProps) {
  const aimagClass = (name: string) =>
    `cursor-pointer transition-all duration-200 ${
      selected?.name === name ? 'fill-[#C2F0D6] stroke-ng stroke-[2px]' : 'fill-[#EDEAE3] stroke-[#C9C6BC]'
    } hover:fill-[#D6F5E3] hover:stroke-ng hover:stroke-[1.5px]`;

  return (
    <svg viewBox="0 0 800 360" className="w-full block rounded-[var(--r)] bg-[var(--bg2)] border border-[var(--bdr)]">
      ...
      <circle cx="393" cy="108" r="5.5" fill="#00C84A" stroke="white" strokeWidth="2" />
      <text x="393" y="100" textAnchor="middle" fontSize="7" fill="#00C84A" fontWeight="700">УБ</text>
      ...
      {viewMode === 'camp' && Object.entries(CAMP_DOTS).map(([province, dots]) =>
        dots.map((dot, i) => (
          <circle
            ...
            cy={dot.cy}
            r={3.5}
            fill={dot.premium ? '#D4A000' : '#00C84A'}
            stroke="white"
            strokeWidth={1.5}
